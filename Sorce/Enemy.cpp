...
int Enemy_Attack_Left() {		//左に弾を打つ
	...
	if (enemy[NUM].ex == player_x && enemy[NUM].ey == player_y) {			//もしplayerに当たったらフラグをONにする
		Player_Hit_Flg = true;
	}
...
int Enemy_Draw() {
	//描画処理
	for (i = 0;i < 4;i++) {
		DrawGraph(enemy[i].x * MAP_SIZE + count_x, enemy[i].y * MAP_SIZE + count_y, EnemyGyallaly[0], true);

		if (Attack_Flg = true) {
			DrawGraph(enemy[i].ex, enemy[i].ey, EnemyShotGyallaly, true);
		}
	}
